...
const Producto = () => {
  ...
  React.useEffect(() => {
    const getProduct = () => {
      fetch(`https://fakestoreapi.com/products/${routeParams.productId}`)
      .then(res => res.json())
      .then(data => { 
        setIsLoading(false);
        setProduct(data)
    ...
    if( routeParams?.productId && routeParams.productId && !product ) 
      getProduct();
      ...
      <>
        ...
        { !isLoading && !product === 0 &&
            <div className='my-10 text-center'>
              <Typography variant="h1" className="text-2xl">Producto no encontrado</Typography>
            </div>
        }
                    ...
                    <div className="lg:flex lg:items-start">
                      <div className="lg:order-2 lg:ml-5">
                        <div className="max-w-xl overflow-hidden rounded-lg">
                          <img className="h-full w-full max-w-full object-cover" src={product.image} alt={product.title} />
                        </div>
                      </div>    
                    </div>
                  </div>
